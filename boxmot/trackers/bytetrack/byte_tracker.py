...
# MAE 
class CenterMask(torch.nn.Module):
    ...
    def forward(self, img):
        ...
        center_h = int(H * 0.5)
        center_w = int(W * 0.5)
        mask[int(center_h - (self.ratio * H) * 0.5): int(center_h + (self.ratio * H) * 0.5), 
             int(center_w - (self.ratio * W) * 0.5): int(center_w + (self.ratio * W) * 0.5)] = 0
        mask = mask.unsqueeze(0).unsqueeze(0).expand(T, B, 1, H, W)
        return mask
...
class MAE_Encoder(torch.nn.Module):
    def __init__(self, image_size=32, patch_size=2, emb_dim=192, num_layer=12, num_head=3, mask_ratio=0.75):
        ...
        self.mask = CenterMask(mask_ratio)
...
class BYTETracker(BaseTracker):
    ...
    def update(self, dets, im=None, embs=None):
        ...
        encoder = MAE_Encoder(mask_ratio=0.5)
        features, mask = encoder(img)
